...
// Declare template
let indexTmpl = Template.Moto_location,
    actionTmpl = Template.Moto_locationAction,
    formTmpl = Template.Moto_locationForm,
    showTmpl = Template.Moto_locationShow;
...
formTmpl.onCreated(function () {
    ...
    this.autorun(() => {
        // Lookup value
        this.subscribe('moto.lookupValue', ['Unit', 'Location Type']);

        let currentData = Template.currentData();
        if (currentData) {
            this.typeState.set(currentData.type);
            this.subscribe('moto.locationById', currentData.locationId);
        }
    });
});
...
AutoForm.addHooks(['Moto_locationForm'], hooksObject);
